...
namespace BILWeb.Stock
{
    public partial class T_Stock_Func : TBase_Func<T_Stock_DB, T_StockInfo>
    {
        ...
        public bool CheckOutStockStatus(ref string strError, T_StockInfo stockModel, string MoveType, string IsEdate)
        {
            ...
            T_AreaInfo areainfo = new T_AreaInfo() { ID = stockModel.AreaID };
            func.GetModelByID(ref areainfo, ref msg);
            if (areainfo.AreaStatus == 2)
            {
                strError = "该库位处于停用状态，不能操作！";
                return false;
            }
            //2018-2-7修改移库去掉限制
            //if (MoveType == "2")
            ...
            if (stockModel.AreaType == 2)
            {
                strError = "该条码处于收货状态，未上架，不能做下架操作！";
                return false;
            }
            //}
