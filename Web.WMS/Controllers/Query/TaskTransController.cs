    ...
    public class TaskTransController : Controller
    {
        ...
        public FileResult Excel(TaskTrans_Model model)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                rowtemp.CreateCell(19).SetCellValue(list[i].vouchertypename);
                rowtemp.CreateCell(20).SetCellValue(list[i].CREATER);
                rowtemp.CreateCell(21).SetCellValue(list[i].CREATETIME.ToString("yyyy/MM/dd HH:mm:ss"));
            }
            // 写入到客户端 
