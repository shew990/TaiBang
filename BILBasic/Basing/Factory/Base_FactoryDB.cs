...
{
    //public abstract class Base_DB<TBase_Model,T> where TBase_Model :   Base_Model where T:class,new()
    public abstract class Base_DB<TBase_Model> where TBase_Model : Base_Model
    {
        ...
        public bool SaveModelListBySqlToDB(User.UserModel user, ref List<TBase_Model> modelList, ref string strError, string strPost = "")
        {
            try
            {
                List<string> lstSql = new List<string>();

                lstSql = this.GetSaveModelListSql(user, modelList, strPost);
        ...
        public string GetNewOrderNo(string strPrex, string tableName, string wmsHeadVourcherNo)
        {
            ...
            string strNewNo = strPrex + DateTime.Now.ToString("yyMMddfff");
        ...
        protected virtual List<string> GetSaveModelListSql(User.UserModel user, List<TBase_Model> modelList, string strPost = "")
        {
            return null;
        }
